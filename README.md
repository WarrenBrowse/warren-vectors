...
| `control.json` | control `/v3` codec (marker `0xC0 0x03`, DAITA capability echo, and the appended route admission messages: `IpRequestRoute` 7, `RouteRejected` 8, `RouteAnchorRequest` 9, `RouteAnchorAck` 10, `RouteEnded` 11) |
...
| `route_admission_v1.json` | route admission by anchor v1 (warren-core doc 107): the API route KEM key pair derived from a signing seed and `key_id`, the anchor and locator HPKE seals (81-byte `SealedToApi`, each with its info, associated data and fixed ephemeral ikm), the derived `anchor_ref` and `route_serial`, and the opens a verifier must refuse (another serial, another exit, an anchor blob as a locator, an unknown key id, a flipped byte). Generated by `scripts/gen_route_admission_v1.py` with the standard-library HPKE of `scripts/hpke_x25519.py`, which the validator reuses to recompute every byte |
| `route_kem_signature_v1.json` | the API's Ed25519 signature over its route KEM key v1 (warren-core doc 107 section 6.5): the 68-byte message (`warren/route-kem-sig/v1` || block version u32 BE || `kem_key_id` || the 32-byte X25519 key || `valid_until` u64 BE), the signature under the server key clients pin, the served `route_admission` block carrying it, and the blocks a client must refuse (a substituted key, another key id, a stretched validity, another version, a key signed by an unpinned key, an expired, unsigned or truncated signature). Signed by `scripts/gen_route_kem_signature_v1.py` with the `cryptography` package; the validator recomputes every signature with the standard-library Ed25519 of `scripts/ed25519_ref.py` |
...
that the Contents table above lists every vector file, and it recomputes the
route admission seals from their inputs. Run it locally with:
